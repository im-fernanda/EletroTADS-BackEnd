...
public class CategoriaResponseDTO extends RepresentationModel<CategoriaResponseDTO> {
    ...
    public void addLinks(Categoria categoria){
        this.add(linkTo(ProdutoCategoriaController.class).slash(categoria.getId()).withSelfRel());

        if(categoria.getProdutos() != null){
            for(Produto produto : categoria.getProdutos()){
                this.add(linkTo(ProdutoCategoriaController.class).slash(produto.getId()).withRel("produto"));
            }
        }
    }
}